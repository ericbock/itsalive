	...
	describe "next gen", ->
		...
		describe "given a 2x2 square", ->
			it "should return the same 2x2 square", ->
				cells.addCell new Point 0, 0
				cells.addCell new Point 0, 1
				cells.addCell new Point 1, 0
				cells.addCell new Point 1, 1
				newCells = game.nextGen(cells)
				newCells.getLiveCount().should.equal 4
				#ugh
				newCells.getCell(new Point 0, 0).isAlive.should.be.true
				newCells.getCell(new Point 0, 1).isAlive.should.be.true
				newCells.getCell(new Point 1, 0).isAlive.should.be.true
				newCells.getCell(new Point 1, 1).isAlive.should.be.true

		describe "given a 1x3 column", ->
			it "should return a 3x1 row with the same center", ->
				cells.addCell new Point 1, 1
				cells.addCell new Point 1, 2
				cells.addCell new Point 1, 3
				newCells = game.nextGen(cells)
				newCells.getLiveCount().should.equal 3
				newCells.getCell(new Point 0, 2).isAlive.should.be.true
				newCells.getCell(new Point 1, 2).isAlive.should.be.true
				newCells.getCell(new Point 2, 2).isAlive.should.be.true
...
describe "Cells", ->
	...
	describe "adding a cell", ->
		...
		it "should return the new cell", ->
			cell = cells.addCell new Point 0, 0
			should.exist(cell)
			cell.should.be.an.instanceof Cell

	describe "get cell", ->
		it "should return a dead cell if no cell exists at the coordinates", ->
			cells.getCell(new Point 100, 100).isAlive.should.be.false

		it "should return the actual cell at the coordinates if it exists", ->
			cells.addCell new Point 100, 100
			cells.getCell(new Point 100, 100).isAlive.should.be.true
	
	describe "liveNeighbors", ->
		it "should return a count of the live neighbors", ->
			cells.addCell new Point 0, 0 # live neighbor
			cells.addCell new Point 2, 2 # live neighbor
			cells.addCell new Point 10, 10 # not a neighbor
			cells.addCell(new Point 0, 2).die() # dead neighbor
			cells.liveNeighbors(new Point 1, 1).should.equal 2
...
describe "A point", ->
	describe "neighbors", ->
		it "should return the positions of the neighbors surrounding a cell", ->
			x = y = 1
			positions = new Point(x, y).neighbors()
